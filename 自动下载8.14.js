...
// 更新内容 8.14：
// 1. 新增“继续下载(非正在下载)”按钮：只恢复当前页面内由“暂停(非正在下载)”批量暂停的任务。
// 2. 单项手动暂停的任务不会被该按钮误恢复；如需全部恢复仍可使用“全部恢复”或单项“恢复”。
// 3. 优化暂停来源记录和界面禁用状态，当前列表没有可恢复任务时按钮自动不可用。
// 4. 修正文件名清理正则中的控制字符写法，避免脚本文件被识别为二进制文本。
// 5. 本文件命名为“自动下载8.14.js”，后续继续沿用“自动下载8.x.js”命名。
// 更新内容 8.13：
// 1. 新增分类文件夹复用功能：检测到本地已存在相似名称的分类文件夹（如"芝恩㱏（共317个）"）时，直接使用该文件夹，不再创建新文件夹（如"芝恩㱏（共319个）"）。
// 2. 只比较分类名称基础部分（去掉"（共N个）"后缀），忽略数量差异。
// 3. 避免因文件数量变化导致创建多个相似的分类文件夹。
// 更新内容 8.12：
// 1. 精简 video 仓库：移除 APK 分析、Node.js 爬虫源码、测试用例、依赖清单和本地依赖环境。
// 2. 仓库仅保留 Tampermonkey 脚本、历史脚本版本、README 和 CHANGELOG 等脚本使用必需内容。
// 3. 本次未改变下载逻辑；本文件命名为"自动下载8.12.js"，后续继续沿用"自动下载8.x.js"命名。
// 更新内容 8.11：
// 1. 新增"暂停(非正在下载)"按钮：仅暂停当前列表/筛选范围内未处于下载中的任务。
// 2. 正在下载中的任务会保持继续下载；等待、准备、重试等待和暂时放弃队列中的任务会被逐项暂停。
// 3. 优化批量暂停逻辑：一次性更新任务状态并只触发一次界面刷新，避免列表大时反复渲染。
// 4. 本文件命名为"自动下载8.11.js"，后续继续沿用"自动下载8.x.js"命名。
// 更新内容 8.10：
// 1. 顶部汇总栏新增已勾选文件总大小，目录扫描完成后会自动显示当前勾选项合计容量。
// 更新内容 8.9：
// 1. 批量下载完成后在顶部显示北京时间、总用时、平均速度、最大速度和最小速度。
// 2. 下载结束后自动在每个大目录文件夹下生成下载报告 TXT，记录完成信息、速度统计和下载目录。
// 更新内容 8.8：
// 1. 整合 Pornhub Video Downloader Plugin v3 v1.0.22 的页面媒体发现规则：普通视频页读取 flashvars_数字.mediaDefinitions，Shorties 页读取 JSON_SHORTIES。
// 2. 自动展开 Pornhub 的 MP4 JSON 清晰度列表和 HLS master playlist，并接入本脚本现有批量下载/m3u8 转 MP4 流程。
// 3. 仅解析当前页面/当前会话已暴露且可访问的媒体定义，不实现扩展里的请求头改写、DRM 或访问控制绕过。
// 更新内容 8.7：
...
(function asmrmoonAudioDownloader() {
  ...
  const state = {
    ...
    saveDirectoryName: '',
    // 新增：缓存已扫描的目录中的分类文件夹，避免重复扫描
    directoryCategoryCache: new Map(),
  };
  ...
  function isPornhubSite() {
    return location.hostname === 'pornhub.com' || location.hostname.endsWith('.pornhub.com');
  }

  function adapterName() {
    if (isAsmrmoonSite()) return 'ASMRMOON目录适配器';
    if (isPornhubSite()) return 'Pornhub页面适配器';
    return '通用页面适配器';
  }
  ...
  function normalizeExposedMediaUrl(value) {
    const text = String(value || '')
      .replace(/\\\//g, '/')
      .replace(/&amp;/g, '&')
      .trim();
  ...
  function mediaNameFromMeta(url, label, kind, meta) {
    const metadata = meta || {};
    const ext = kind === 'm3u8' ? 'm3u8' : extension(url) || (kind === 'video' ? 'mp4' : '');
    const parts = [
      metadata.title || label || document.title,
      metadata.quality,
      metadata.format && metadata.format !== ext ? metadata.format : '',
    ]
      .map(safeNamePart)
      .filter(Boolean);
    let name = safeNamePart(metadata.name || parts.join(' - ')) || nameFromMediaUrl(url, label);
    if (ext && !new RegExp(`\\.${ext}$`, 'i').test(name)) name = `${name}.${ext}`;
    return trimFileName(name);
  }

  function addGenericMediaUrl(rawUrl, label, sourceHint, order, meta) {
    ...
    const item = {
      ...
      name: metadata.title || metadata.quality || metadata.name ? mediaNameFromMeta(url, label, kind, metadata) : nameFromMediaUrl(url, label),
      kind,
      size: 0,
      modified: '',
      sign: '',
  ...
  function getPornhubShortiesVkey(url) {
    const match = String(url || '').match(/\/shorties\/([^/?#]+)\/?$/);
    return match ? match[1] || '' : '';
  }

  function getPornhubJsonShortiesArray() {
    let raw;
    try {
      raw = mainPageWindow().JSON_SHORTIES;
    } catch (error) {
      raw = window.JSON_SHORTIES;
    }
    if (!raw) return null;
  ...
  function findPornhubShortiesItem(list, vkey) {
    if (!Array.isArray(list) || !vkey) return null;
    const byVkey = list.find((item) => item && item.vkey === vkey);
    if (byVkey) return byVkey;
    return list.find((item) => {
      if (!item) return false;
      return String(item.linkUrl || '').includes(vkey) || String(item.shortieUrl || '').includes(vkey);
    }) || null;
  }

  function extractJsonArrayFromHtml(html, bracketStart) {
    let depth = 1;
    let inString = false;
    let escape = false;
    for (let index = bracketStart + 1; index < html.length; index += 1) {
      ...
      if (depth === 0) {
        try {
          return JSON.parse(html.slice(bracketStart, index + 1));
        } catch (error) {
          return null;
        }
      }
  ...
  function extractPornhubJsonShortiesArrayFromHtml(html) {
    const exactNeedle = 'JSON_SHORTIES = insertAfterNthPosition([';
    const exactIndex = String(html || '').indexOf(exactNeedle);
    if (exactIndex >= 0) return extractJsonArrayFromHtml(String(html), exactIndex + exactNeedle.length - 1);

    const looseMatch = String(html || '').match(/JSON_SHORTIES\s*=\s*insertAfterNthPosition\s*\(/);
    if (!looseMatch || looseMatch.index === undefined) return null;
    const bracketStart = String(html).indexOf('[', looseMatch.index);
    return bracketStart >= 0 ? extractJsonArrayFromHtml(String(html), bracketStart) : null;
  }

  function findPornhubFlashvarsName() {
    const page = mainPageWindow();
    const scriptSelectors = ['#mobileContainer > script:nth-child(1)', '#player > script:nth-child(1)', 'script'];
    for (const selector of scriptSelectors) {
      const scripts = selector === 'script' ? Array.from(document.scripts || []) : Array.from(document.querySelectorAll(selector));
      for (const script of scripts) {
        const match = String(script.textContent || script.innerHTML || '').match(/flashvars_\d+/);
        if (!match) continue;
        try {
          if (page[match[0]] && Array.isArray(page[match[0]].mediaDefinitions)) return match[0];
        } catch (error) {
          // Keep looking.
        }
      }
    ...
    try {
      return Object.keys(page).find((key) => /^flashvars_\d+$/.test(key) && page[key] && Array.isArray(page[key].mediaDefinitions)) || '';
    } catch (error) {
      return '';
    }
  }

  async function getPornhubMediaDefinitions() {
    const vkey = getPornhubShortiesVkey(location.href);
    if (vkey) {
      const localList = getPornhubJsonShortiesArray();
      const localItem = findPornhubShortiesItem(localList, vkey);
      if (localItem && Array.isArray(localItem.mediaDefinitions)) return localItem.mediaDefinitions;

      try {
        const html = await requestText(location.href);
        const fetchedList = extractPornhubJsonShortiesArrayFromHtml(html);
        const fetchedItem = findPornhubShortiesItem(fetchedList, vkey);
        if (fetchedItem && Array.isArray(fetchedItem.mediaDefinitions)) return fetchedItem.mediaDefinitions;
      } catch (error) {
        console.warn('[Universal Media Downloader] Pornhub Shorties HTML parse failed:', error);
      }
    }

    const flashvarsName = findPornhubFlashvarsName();
    if (!flashvarsName) return [];
    try {
      const mediaDefinitions = mainPageWindow()[flashvarsName].mediaDefinitions;
      return Array.isArray(mediaDefinitions) ? mediaDefinitions : [];
    } catch (error) {
      return [];
    }
  }

  function pornHubPageTitle() {
    const uploader = document.querySelector('.video-actions-container .usernameBadgesWrapper a, .userInfoContainer a');
    const uploaderText = uploader ? safeNamePart(uploader.textContent || uploader.innerHTML || '') : '';
    const title = safeNamePart(document.title || 'video');
    return uploaderText ? `${uploaderText} - ${title}` : title;
  }

  function qualityFromM3u8Variant(variant, fallback) {
    const resolution = String((variant && variant.resolution) || '');
    const heightMatch = resolution.match(/x(\d{3,4})$/i);
    if (heightMatch) return `${heightMatch[1]}p`;
    const bandwidth = Number((variant && variant.bandwidth) || 0);
    if (bandwidth > 0) return `${Math.round(bandwidth / 1000)}kbps`;
    return fallback || 'hls';
  }

  function normalizePornhubMediaEntry(entry, fallbackTitle) {
    if (!entry || typeof entry !== 'object') return null;
    const url = normalizeExposedMediaUrl(entry.videoUrl || entry.url || entry.src);
    if (!url || !isMediaUrl(url)) return null;
    const format = String(entry.format || '').toLowerCase() === 'hls' ? 'm3u8' : (entry.format || extension(url) || '');
    return {
      videoUrl: url,
      quality: String(entry.quality || entry.defaultQuality || entry.name || '').trim() || format || 'media',
      format: format || (kindFromMediaUrl(url) === 'm3u8' ? 'm3u8' : 'mp4'),
      title: fallbackTitle,
    };
  }

  async function expandPornhubMp4Definition(definition, title) {
    const url = normalizeExposedMediaUrl(definition && definition.videoUrl);
    if (!url) return [];
    try {
      ...
      const list = Array.isArray(parsed)
        ? parsed
        : Array.isArray(parsed && parsed.media)
          ? parsed.media
          : Array.isArray(parsed && parsed.data)
            ? parsed.data
            : [];
      return list
        .map((entry) => normalizePornhubMediaEntry(entry, title))
        .filter(Boolean);
    } catch (error) {
      const direct = normalizePornhubMediaEntry(definition, title);
      return direct ? [direct] : [];
    }
  }

  async function expandPornhubHlsDefinition(definition, title) {
    const masterUrl = normalizeExposedMediaUrl(definition && definition.videoUrl);
    if (!masterUrl || !isMediaUrl(masterUrl)) return [];
    try {
      const text = await requestText(masterUrl);
      const playlist = parseM3u8Playlist(text, masterUrl);
      if (!playlist.variants.length) {
        return [{
          videoUrl: masterUrl,
          quality: String(definition.quality || 'hls'),
          format: 'm3u8',
          title,
        }];
      }
      return playlist.variants.map((variant) => ({
        ...
        quality: qualityFromM3u8Variant(variant, definition.quality),
        format: 'm3u8',
        title,
      }));
    } catch (error) {
  ...
  async function collectPornhubMediaEntries() {
    if (!isPornhubSite()) return [];
    const definitions = await getPornhubMediaDefinitions();
    if (!Array.isArray(definitions) || !definitions.length) return [];

    const title = pornHubPageTitle();
    const entries = [];
    const mp4Definition = definitions.find((item) => item && String(item.format || '').toLowerCase() === 'mp4' && item.videoUrl);
    if (mp4Definition) entries.push(...await expandPornhubMp4Definition(mp4Definition, title));

    const directDefinitions = definitions.filter((item) => item && item.videoUrl && item !== mp4Definition && String(item.format || '').toLowerCase() !== 'hls');
    directDefinitions.forEach((item) => {
      const entry = normalizePornhubMediaEntry(item, title);
      if (entry) entries.push(entry);
    });

    const hlsDefinitions = definitions.filter((item) => item && item.videoUrl && String(item.format || '').toLowerCase() === 'hls');
    for (const definition of hlsDefinitions) {
      entries.push(...await expandPornhubHlsDefinition(definition, title));
    }

    const seen = new Set();
    return entries.filter((entry) => {
      if (!entry || !entry.videoUrl || seen.has(entry.videoUrl)) return false;
  ...
  async function pickSaveDirectory() {
    ...
    state.directoryCategoryCache.clear(); // 清空缓存
  ...
  /**
   * 从文件夹名称中提取原始分类名（去掉"（共N个）"部分）
   * 例如："芝恩㱏（共317个）" -> "芝恩㱏"
   */
  function extractBaseCategoryName(folderName) {
    const match = String(folderName || '').match(/^(.+?)（共\d+个）$/);
    if (match) return match[1];
    return folderName || '';
  }

  /**
   * 检查两个文件夹名称是否属于同一个分类（忽略数量差异）
   */
  function isSameCategoryFolder(existingFolder, targetFolder) {
    return extractBaseCategoryName(existingFolder) === extractBaseCategoryName(targetFolder);
  }
   ...
   * 使用缓存避免重复扫描，提高性能
   */
  async function scanAndCacheCategories(directory) {
    if (!directory) return;

    // 为这个目录生成唯一缓存键
    // 注意：由于 File System API 不直接提供完整路径，我们使用目录名称作为简化键
    // 在实际使用中，我们会按层级缓存
    const cacheKey = directory.name || 'root';

    // 如果已缓存，跳过扫描
    if (state.directoryCategoryCache.has(cacheKey)) {
      return state.directoryCategoryCache.get(cacheKey);
    }
    ...
    try {
      for await (const [entryName, entryHandle] of directory.entries()) {
        if (entryHandle.kind === 'directory') {
          // 提取基础分类名
          const baseName = extractBaseCategoryName(entryName);
          if (baseName) {
            // 如果这个基础分类名已有文件夹，保留第一个发现的
            if (!categoryMap.has(baseName)) {
              categoryMap.set(baseName, { name: entryName, handle: entryHandle });
            }
          }
        }
      }
    } catch (error) {
      console.warn('[Universal Media Downloader] 扫描目录失败:', error);
    }

    state.directoryCategoryCache.set(cacheKey, categoryMap);
    return categoryMap;
  }

  /**
   * 查找目录中与目标分类名称相同的现有文件夹
   * 使用缓存提高性能
   */
  async function findExistingCategoryFolder(directory, targetFolderName) {
    if (!directory || !targetFolderName) return null;

    const targetBaseName = extractBaseCategoryName(targetFolderName);
    if (!targetBaseName) return null;

    // 从缓存中获取已扫描的分类文件夹
    const categoryMap = await scanAndCacheCategories(directory);
    if (!categoryMap) return null;

    // 查找是否有相同基础分类名的文件夹
    return categoryMap.get(targetBaseName) || null;
  }

  /**
   * 清空目录缓存（在下载开始或目录变更时调用）
   */
  function clearDirectoryCache() {
    state.directoryCategoryCache.clear();
  }
  ...
  async function createWritableForDownloadName(name) {
    ...
    for (const part of parts) {
      // 检查是否存在相同分类名称的现有文件夹
      const existingFolder = await findExistingCategoryFolder(directory, part);

      if (existingFolder) {
        // 找到相似名称的文件夹，使用它而不是创建新的
        directory = existingFolder.handle;
      } else {
        // 没有找到，创建新目录
        directory = await directory.getDirectoryHandle(part, { create: true });

        // 创建后更新缓存
        const cacheKey = directory.parent?.name || root.name || 'root';
        if (!state.directoryCategoryCache.has(cacheKey)) {
          state.directoryCategoryCache.set(cacheKey, new Map());
        }
        const baseName = extractBaseCategoryName(part);
        if (baseName) {
          state.directoryCategoryCache.get(cacheKey).set(baseName, { name: part, handle: directory });
        }
      }
    }
  ...
  function beijingTimeText(timestamp) {
  ...
  function selectedVisibleSize(items) {
    return items
      .filter((item) => state.selectedPaths.has(item.path))
  ...
  async function scanGenericPage() {
    ...
    state.inactivePausedPaths.clear();
    ...
    try {
      ...
      let siteSpecificCount = 0;
      if (isPornhubSite()) {
        state.status = '正在解析 Pornhub 页面媒体定义...';
        render();
        try {
          const entries = await collectPornhubMediaEntries();
          entries.forEach((entry, index) => {
            addGenericMediaUrl(entry.videoUrl, entry.title || document.title, 'pornhub-mediaDefinitions', urls.length + index, entry);
          });
          siteSpecificCount = entries.length;
        } catch (error) {
          console.warn('[Universal Media Downloader] Pornhub mediaDefinitions scan failed:', error);
        }
      }
      state.scannedFiles = state.items.size;
      state.status = siteSpecificCount
        ? `完成：当前页面发现媒体 ${state.items.size} 个（含 Pornhub 清晰度 ${siteSpecificCount} 条）`
        : `完成：当前页面发现媒体 ${state.items.size} 个`;
    } catch (error) {
  ...
  function pauseTaskByPath(path) {
    ...
    state.inactivePausedPaths.delete(path);
  ...
  function isFinishedDownloadTask(task) {
    return Boolean(task && (task.status === 'done' || task.status === 'failed' || task.status === 'skipped'));
  }

  function isNonDownloadingPausableTask(task) {
    return Boolean(task && !task.paused && !isFinishedDownloadTask(task) && task.status !== 'downloading');
  }

  function visibleNonDownloadingPausableCount(items) {
    return (items || visibleItems()).reduce((count, item) => {
      const task = state.downloadTasks.get(item.path);
      return count + (isNonDownloadingPausableTask(task) ? 1 : 0);
    }, 0);
  }

  function isInactivePausedResumableTask(item, task) {
    return Boolean(item && task && task.paused && state.inactivePausedPaths.has(item.path) && !isFinishedDownloadTask(task) && task.status !== 'downloading');
  }

  function visibleInactivePausedResumableCount(items) {
    return (items || visibleItems()).reduce((count, item) => {
      const task = state.downloadTasks.get(item.path);
      return count + (isInactivePausedResumableTask(item, task) ? 1 : 0);
    }, 0);
  }

  function pauseVisibleNonDownloadingTasks() {
    ...
    items.forEach((item) => {
      const task = state.downloadTasks.get(item.path);
      if (!task || isFinishedDownloadTask(task)) return;
      ...
      task.paused = true;
      task.speed = 0;
      state.pausedPaths.add(item.path);
      state.inactivePausedPaths.add(item.path);
    });

    state.status = paused
      ? `已暂停当前列表 ${paused} 个非下载中任务；${active} 个正在下载任务保持继续`
      : `当前列表没有可暂停的非下载中任务；${active} 个正在下载任务保持继续`;
    renderDownloadProgress(true);
  }

  function resumeVisibleInactivePausedTasks() {
    if (!state.downloading) return;
    const items = visibleItems();
    let resumed = 0;
    ...
    items.forEach((item) => {
      const task = state.downloadTasks.get(item.path);
      if (!isInactivePausedResumableTask(item, task)) return;
      task.paused = false;
      state.pausedPaths.delete(item.path);
      state.inactivePausedPaths.delete(item.path);
      resumed += 1;
    });

    state.status = resumed
      ? `已继续当前列表 ${resumed} 个非下载中暂停任务`
      : '当前列表没有由“暂停(非正在下载)”暂停的可恢复任务';
    renderDownloadProgress(true);
  }
  ...
  function hexToBytes(hex) {
    ...
    for (let index = 0; index < 16; index += 1) {
      bytes[index] = Number.parseInt(clean.slice(index * 2, index + 2), 16) || 0;
    }
    return bytes;
  }
  ...
  function resetDownloadTasks(items, categoryCounts) {
    ...
    state.downloadCompletedAt = 0;
    state.downloadSpeedMin = 0;
    state.downloadSpeedMax = 0;
    state.downloadSpeedSampleCount = 0;
    state.lastProgressRender = 0;
    state.globalPaused = false;
    // 清空目录缓存以便重新扫描
    clearDirectoryCache();
  ...
  function recordDownloadSpeedSample(speed) {
    const value = Number(speed || 0);
    if (!state.downloading || state.downloadActive <= 0 || value <= 0) return;
    state.downloadSpeedMin = state.downloadSpeedSampleCount
      ? Math.min(state.downloadSpeedMin, value)
      : value;
    state.downloadSpeedMax = Math.max(state.downloadSpeedMax, value);
    state.downloadSpeedSampleCount += 1;
  }
  ...
  function updateDownloadStats() {
    ...
    recordDownloadSpeedSample(state.downloadSpeed);
  ...
  function downloadTaskStatusText(task) {
    ...
    if (task.status === 'skipped') return '跳过';
    if (task.status === 'failed') return '失败';
    if (task.status === 'deferred') return '暂缓未完成';
    if (task.status === 'queued' || task.status === 'starting') return '未开始';
    return task.status || '未知';
  }

  function outputDirectoryPart(outputName) {
    const parts = String(outputName || '').split(/[\\/]/).filter(Boolean);
    parts.pop();
    return parts.join('/') || '根目录';
  }

  function topOutputDirectoryPart(outputName) {
    const parts = String(outputName || '').split(/[\\/]/).filter(Boolean);
    return parts.length > 1 ? parts[0] : '';
  }

  function downloadReportFileName(timestamp) {
    const stamp = beijingTimeText(timestamp).replace(/\D/g, '').slice(0, 14);
    return `下载报告-${stamp || Date.now()}.txt`;
  }

  function buildDownloadCompletionInfo(stopped, baseStatus) {
    const completedAt = Date.now();
    state.downloadCompletedAt = completedAt;
    const startedAt = state.downloadStartedAt || completedAt;
    const elapsedMs = Math.max(0, completedAt - startedAt);
    const averageSpeed = elapsedMs > 0 ? (state.downloadBytesLoaded * 1000) / elapsedMs : 0;
    const maxSpeed = state.downloadSpeedSampleCount ? state.downloadSpeedMax : averageSpeed;
    const minSpeed = state.downloadSpeedSampleCount ? state.downloadSpeedMin : averageSpeed;
    const finishedAtText = beijingTimeText(completedAt);
    const elapsed = durationText(elapsedMs);
    const resultLabel = stopped ? '下载已停止' : '下载完成';
    const speedText = `平均速度 ${readableSpeed(averageSpeed)}，最大速度 ${readableSpeed(maxSpeed)}，最小速度 ${readableSpeed(minSpeed)}`;
    const shortText = `${resultLabel}：北京时间 ${finishedAtText} · 总用时 ${elapsed} · ${speedText.replace(/，/g, ' · ')}`;
  ...
  function buildDownloadReportText(info) {
    const tasks = Array.from(state.downloadTasks.values()).sort((a, b) => Number(a.index || 0) - Number(b.index || 0));
    const directories = Array.from(new Set(tasks.map((task) => outputDirectoryPart(task.outputName || task.name)).filter(Boolean))).sort((a, b) => a.localeCompare(b, 'zh-Hans-CN'));
    const lines = [
      '下载报告',
      '',
      `结果：${info.resultLabel}`,
      `下载完成北京时间：${info.finishedAtText}`,
      `总用时：${info.elapsed}`,
      `平均速度：${readableSpeed(info.averageSpeed)}`,
      `最大速度：${readableSpeed(info.maxSpeed)}`,
      `最小速度：${readableSpeed(info.minSpeed)}`,
      `总下载量：${readableTransferSize(state.downloadBytesLoaded)}`,
      `总任务：${state.downloadTotal}`,
      `成功：${state.downloadDone}`,
      `跳过：${state.downloadSkipped}`,
      `失败：${state.downloadFailed}`,
      `下载模式：${downloadModeText()}`,
      `保存根目录：${state.saveDirectoryName || '未选择'}`,
      `扫描/下载来源：${state.rootPath}`,
      '',
      '下载的目录：',
      ...(directories.length ? directories.map((directory) => `- ${directory}`) : ['- 根目录']),
      '',
      '任务明细：',
    ];

    tasks.forEach((task, index) => {
      lines.push(`${index + 1}. [${downloadTaskStatusText(task)}] ${task.outputName || task.name || task.path}`);
      lines.push(`   源路径：${task.path || '-'}`);
      lines.push(`   输出目录：${outputDirectoryPart(task.outputName || task.name)}`);
      lines.push(`   大小：${readableTransferSize(Math.max(Number(task.loaded || 0), Number(task.size || 0)))}`);
      if (task.error) lines.push(`   备注：${task.error}`);
    });

    return `${lines.join('\n')}\n`;
  }

  async function writeTextDownloadFile(name, text) {
    let writer = null;
    try {
      writer = await createWritableForDownloadName(name);
      await writer.write(new Blob([text], { type: 'text/plain;charset=utf-8' }));
      await writer.close();
    } catch (error) {
      await abortWritable(writer);
      throw error;
  ...
  async function writeDownloadReports(info) {
    if (!state.saveDirectoryHandle) return 0;
    const tasks = Array.from(state.downloadTasks.values());
    const topDirectories = new Set(tasks.map((task) => topOutputDirectoryPart(task.outputName || task.name)).filter(Boolean));
    if (!topDirectories.size || tasks.some((task) => !topOutputDirectoryPart(task.outputName || task.name))) {
      topDirectories.add('');
    }
    const reportText = buildDownloadReportText(info);
    const fileName = downloadReportFileName(info.completedAt);
    let count = 0;

    for (const directory of topDirectories) {
      const reportPath = directory ? `${directory}/${fileName}` : fileName;
      await writeTextDownloadFile(reportPath, reportText);
      count += 1;
    }
  ...
  async function completeDownloadBatch(stopped, baseStatus) {
    state.downloading = false;
    updateDownloadStats();
    const info = buildDownloadCompletionInfo(stopped, baseStatus);
    const finalStatus = `${baseStatus}；完成时间：${info.finishedAtText}；总用时：${info.elapsed}；${info.speedText}`;
    state.lastCompletionText = info.shortText;
    state.status = finalStatus;
    render();

    try {
      state.status = `${finalStatus}；正在写入下载报告...`;
      render();
      const reportCount = await writeDownloadReports(info);
      state.status = reportCount
        ? `${finalStatus}；报告已写入 ${reportCount} 个大目录`
        : `${finalStatus}；未选择保存目录，未写入报告`;
    } catch (error) {
      state.status = `${finalStatus}；报告写入失败：${error.message || error}`;
    }
    render();
  }
  ...
  async function downloadVisible() {
    ...
    if (!queuedTaskCount()) {
      await completeDownloadBatch(false, `下载完成：成功 ${state.downloadDone} 个，跳过 ${state.downloadSkipped} 个，失败 ${state.downloadFailed} 个`);
      return;
    }
    ...
    const baseStatus = state.stopDownload
    ...
    await completeDownloadBatch(Boolean(state.stopDownload), baseStatus);
  }
  ...
  function ensureUi() {
    ...
    ui.pauseInactive = button('暂停(非正在下载)');
    ui.resumeInactive = button('继续下载(非正在下载)');
    ...
    ui.panel.append(head, bar, controls, ui.report, ui.summary, ui.list, foot);
    ...
    ui.pauseInactive.addEventListener('click', pauseVisibleNonDownloadingTasks);
    ui.resumeInactive.addEventListener('click', resumeVisibleInactivePausedTasks);
  ...
  function render() {
    ...
    const selectedSize = selectedVisibleSize(items);
    ...
    ui.report.textContent = state.lastCompletionText;
    ui.report.classList.toggle('visible', Boolean(state.lastCompletionText));
    ...
    ui.summary.textContent = `适配器：${state.adapterName} · 当前路径：${state.rootPath} · 已扫描目录 ${state.scannedDirs} · 已扫描文件 ${state.scannedFiles} · 媒体 ${state.items.size} · 当前显示 ${items.length} · 已勾选 ${selectedCount}（合计 ${readableTransferSize(selectedSize)}） · 下载模式：${modeText}${directoryText}${downloadText}${skippedText}`;
    ...
    ui.pauseInactive.disabled = !state.downloading || !visibleNonDownloadingPausableCount(items);
    ui.resumeInactive.disabled = !state.downloading || !visibleInactivePausedResumableCount(items);
  ...
  function row(item) {
    ...
    pause.disabled = !task || !state.downloading || isFinishedDownloadTask(task);
