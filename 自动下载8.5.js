...
(function asmrmoonAudioDownloader() {
  ...
  function pathSmallDirectoryNameFor(item) {
    if (!item || item.source === 'generic') return '';
    const parts = normalizePath(item.path || joinPath(item.parent || '/', item.name))
      .split('/')
      .filter(Boolean);
    const parentParts = parts.slice(0, -1);
    if (parentParts.length < 3) return '';
    return safeNamePart(stripKnownMediaExtension(parentParts[2]));
  }

  function smallDirectoryNameFor(item) {
    const displayName = item && (item.saveName || displayNameFor(item));
    if (!categoryNameFor(displayName)) return '';
    const fromPath = pathSmallDirectoryNameFor(item);
    if (fromPath) return fromPath;
    const parts = displayTitleParts(displayName);
    if (parts.length < 3) return '';
    return safeNamePart(stripKnownMediaExtension(parts[2]));
  }
  ...
  function pauseAllDownloads() {
    ...
    state.downloadTasks.forEach((task) => {
      if (task.status === 'queued' || task.status === 'downloading' || task.status === 'retrying' || task.status === 'starting') {
        task.speed = 0;
      }
    });
  ...
  function updateDownloadStats() {
    ...
    if (state.downloading) {
      ...
      const totalText = state.downloadBytesTotal
        ...
        : '';
      state.status = `下载中：完成 ${finished}/${state.downloadTotal}${skippedText}，进行 ${state.downloadActive} 个，速度 ${readableSpeed(state.downloadSpeed)}${totalText}`;
    }
  }
  ...
  function startDownloadTask(task, retryCount) {
    const now = Date.now();
  ...
  function claimNextDownload(items) {
    for (let index = 0; index < items.length; index += 1) {
      const item = items[index];
      const task = state.downloadTasks.get(item.path);
      if (!task || task.status !== 'queued' || task.paused) continue;
  ...
  async function runDownloadTask(item, task, categoryCounts) {
    for (let attempt = 0; attempt <= MAX_DOWNLOAD_RETRIES; attempt += 1) {
      try {
        await waitForResume(task);
        startDownloadTask(task, attempt);
        ...
        const canRetry = attempt < MAX_DOWNLOAD_RETRIES && retryableError(error);
        if (!canRetry) {
  ...
  async function downloadVisible() {
    ...
    const categoryText = categoryCounts.size ? `\n\n将按名称中间段保存到 ${categoryCounts.size} 个大目录，并自动识别小目录标题，例如：奶斯姐姐（共168个）/L9 学姐的温柔控制 -奶斯/文件名。` : '';
